...
class recupData:
    ...
    def recupHoraires(self, semestre, semestre_onglet):
        print("recupHoraires appelé")
        planning = pd.ExcelFile('Documents/Planning 2023-2024.xlsx')
        self.cursor.execute("SELECT Num_Res, Dates FROM Horaires WHERE Semestre = ?", (semestre,))
            ...
            for index, row in S.iterrows():
                if num_res in row.values:
                    ...
                    if index_num_res + 12 < len(row) and row.iloc[index_num_res + 3] >= 0:
                        ...
                        print(f"12e case: {valeur_case_12}")
                        insertdata = insertData()
                        insertdata.insert_horaires(semestre, dates, valeur_case_3, valeur_case_5, valeur_case_7, valeur_case_12)
