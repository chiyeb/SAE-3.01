...
def concordance(semestre):
    ...
    cursor.execute("SELECT * FROM Maquette WHERE Semestre = ?", (semestre,))
    rslt_maquette = cursor.fetchall()
    cursor.execute("SELECT * FROM Planning WHERE Semestre = ?", (semestre,))
    rslt_planning = cursor.fetchall()
    cpt = 0
